	...
	*/
	...
	function getLogo(){
		...
		?>
            ...
            $url_base = JURI::base()."index.php/".$lang_prefix;
            }
			?>
			<a class="logo" href="<?php echo $url_base; ?>" title="<?php echo $app->getCfg('sitename'); ?>">
			...
			if($position['class']!=''){
				...
				if($countModules < $countTag){
					if ($doc->countModules($position['value']) != 0 || $position['type'] == 'feature' ) {
						...
						if($k == $countModules) {
							if($_total >= 12) $cls_end = 12 - ($_total - 12) ;
							else $cls_end = 12 - $_total;
	...
	function renFeaturePos ($position, $elementstyle, $elementclass='' ) {
		?>
		<div id="<?php echo "yt_".strtolower(substr($position['value'], 1));?>" <?php echo $elementclass; ?> <?php echo $elementstyle; ?> >
			<?php
